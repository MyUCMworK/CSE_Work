...
// This program is let us fill in two function, which are value_roll_hand function and heuristic function.
// Value_roll_hand function considers every possible outcome of the dice roll and 
// the resulting utility value of each ofthose possible outcomes. If dice are going to invalid, we skip it. 
// If there are no invalid, calculate the probability of the symbol that each combination of the dice, and the value of symbol.
// In heuristic function, Calculate the different values between the value of user and computer
// Computer has more if diff is positive, and user has more if diff is negative, and same if they have same number of the brains.
// By using the formal diff / 13 and times 100, the computer can got heuristic value, and that is the probability of the win or failed, also it can show computer the how is the current state going. 
// The heuristic value can help computer to make decision to roll or stop.
...
public class Eval {
	...
	// YOUR CODE SHOULD CALL "value_rolled_hand" AT SOME POINT.
		//Those loops consider every possible outcome of the dice roll and the resulting utility value of each ofthose possible outcomes.
		//If dice are going to invalid, we skip it. 
		//Also, calculate the probability of the symbol that each combination of the dice, and the value of symbol.
		for (int i = 0; i < DieFace.values().length; i++){ // first die rolled 
			DieFace die_1 = DieFace.values()[i]; // When first die is rolled, we got first symbol
			if (die_1 == DieFace.invalid){ // if first die is invalid, we skip it by using continue and re-roll first die
				continue;
			}
			for (int j = 0; j < DieFace.values().length; j++){ // first die rolled completely, second die rolled
				DieFace die_2 = DieFace.values()[j]; // When second die is rolled, we got second symbol
				if (die_2 == DieFace.invalid){ // if second die is invalid, we skip it by using continue and re-roll second die
					continue;
				}
				for (int k = 0; k < DieFace.values().length; k++){ // first and second dice rolled completely, thired die rolled
					DieFace die_3 = DieFace.values()[k]; // When third die is rolled, we got third symbol
					if (die_3 == DieFace.invalid){ // if third die is invalid, we skip it by using continue and re-roll first die
						continue;
					}else{ // if three dice are all not invalid, calculate the probability of the symbol that each combination of the dice
						double p = s.rollProb(die_1, die_2, die_3); // calculate the probability of the symbol that each combination of the dice
						s.roll(die_1, die_2, die_3); // roll those dice
						double roll_value = value_rolled_hand(s, depth); // call the value_rolled_hand function which can compute the expected utility value of this state
						val = val + (roll_value * p); // calculate the value of symbol from the rolls
					}
				}
			}
		}
    ...
    //              is returned.
    static public double heuristic (State s) { // Calculatet the heuristic value, and it can help computer to make decision to roll or stop.
	...
	// Calculate the different between the value of user and computer
	// computer has more if diff is positive, and user has more if diff is negative, and same if they have same number of the brains
	double diff = s.comp_brains_eaten - s.user_brains_eaten; 
	// If someone got value of 13 or more, this round will be final round unless user and computer both have 13 or more. 
	// The diff / 13 and times 100 is the heuristic value, and that is the probability of the win or failed, also it can show computer the how is the current state going. 
	// The heuristic value can help computer to make decision to roll or stop.
	value = (diff / 13.0) * 100;
