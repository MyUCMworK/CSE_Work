...
public class Layer {
	...
	// PLACE YOUR CODE HERE ...
		Vector error = targ.difference(act); // the difference of the target and activation is error, which means that error = target - activation
		Vector der = net.derivative(min, max); // doing gradient descent, which is partial derivative of the function g, with the min and max values
	
		for (int i = 0; i < n; i++){ // this for loop will go through the iterations until n
			double dererr = error.get(i) * der.get(i); // multiplies the value of error and der to get the output called dereer
			delta.set(i, dererr); // sets the output
		}
	...
	// PLACE YOUR CODE HERE ...
		Vector error = new Vector(n, 0.0); //
	
		for (int i = 0; i < outputs.size(); i++){ // this for loop will get the amount of times of the projection
			Projection p = outputs.get(i); // this for loop will get the amount of times of the projection
			Vector temp = p.W.transpose().product(p.output.delta); // this will find the summed product by rotating the Matrix W and multiplying it with the delta of p
			error = error.sum(temp); // the final solution which is the summation of the error
		}
